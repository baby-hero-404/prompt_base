...
# -*- coding: utf-8 -*-
"""
UX/UI Pro Max Search - BM25 search engine for UX/UI style guides
...
import sys
import io
from core import CSV_CONFIG, AVAILABLE_STACKS, MAX_RESULTS, search, search_stack, sanitize_filename
...
def format_output(result):
    ...
    output = []
    if result.get("stack"):
        output.append(f"## UX/UI Pro Max Stack Guidelines")
        output.append(f"**Stack:** {result['stack']} | **Query:** {result['query']}")
    else:
        output.append(f"## UX/UI Pro Max Search Results")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="UX/UI Pro Max Search")
    ...
    parser.add_argument("--page", type=str, default=None, help="Create page-specific override file in docs/design-system/pages/")
    ...
    if args.design_system:
        ...
        if args.persist:
            ...
            if args.page:
                page_slug = sanitize_filename(args.page)
                print(f"   📄 docs/design-system/{project_slug}/pages/{page_slug}.md (Page Overrides)")
            print("")
